...
describe('global utility functions', () => {
  ...
  it('replaceAsync performs asynchronous replacements', async () => {
    const result = await replaceAsync('a1b2', /\d/g, async (m) =>
      String(Number(m) + 1),
    )
  ...
  it('detects mime type from byte header', () => {
    expect(getFileMimeType(Uint8Array.from([0x89, 0x50, 0x4e, 0x47]))).toBe(
      'image/png',
    )
    expect(getFileMimeType(Uint8Array.from([0xff, 0xd8, 0xff, 0xe0]))).toBe(
      'image/jpeg',
    )
    expect(getFileMimeType(Uint8Array.from([0x00, 0x00, 0x00, 0x00]))).toBe(
      'audio/mp3',
    )
  })
})
